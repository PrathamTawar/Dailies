...
{
    let data = await axios.get('http://127.0.0.1:8000/api/get')
    if (filter === 'all')
    {
...
todoAll.addEventListener('click', async (e) => {
    ...
    if (clickedBtn.classList.contains('todo-checkbox')) {
        ...
        await axios.put(`http://127.0.0.1:8000/api/check/${itemId}`)
    }
});
...
addInpt.addEventListener('submit', (e)=> {
    ...
    let mytitle = e.target[0].value
    let mydescription = e.target[1].value
...
{
    await axios.post('http://127.0.0.1:8000/api/set', task)
    getItems()  
}